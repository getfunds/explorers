...
export default function AddressInput() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      const response = await axios.post('/api/metadata', { address: address });
  ...
  return (
    ...
    <div className="flex w-full h-full flex-col justify-center items-center xl:p-16 p-4 pt-8 space-y-24">
      <form onSubmit={handleSubmit} className="flex w-full items-center justify-center">
        ...
        className="flex p-2 rounded-r-lg bg-zinc-800 font-bold text-white duration-200 cursor-pointer" type="submit">
      ...
      {enter ? ( 
        submit ?  ( 
          ...
          <NFT tokens={balance} address={address}  /> 
        ) : ( 
          <svg
            aria-hidden="true"
